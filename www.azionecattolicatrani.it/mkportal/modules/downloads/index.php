...
*/
  ...
  function submit_comment() {
	...
	function submit_rate() {
		...
		$t_ev1 = $row['idcategoria'];
		$t_ev2 = $this->retrieve_event($t_ev1);
		$navbar = "<a href=\"index.php?ind=downloads\">{$mklib->lang['dw_ptitle2']}</a>-><a href=\"index.php?ind=downloads&amp;op=section_view&amp;idev=$t_ev1\">$t_ev2</a>-><a href=\"index.php?ind=downloads&amp;op=entry_view&amp;iden=$t_id\">$t_t</a>-><a href=\"#\">{$mklib->lang['dw_vote']}</a>";
	   $content .= "
		<tr>
		  <td class=\"modulecell\">
	...
	function del_comment() {
		...
		function download_file() {
			...
			$ide= $mkportals->input['ide'];
			$query = $DB->query( "SELECT downloads FROM mkp_download where id = '$ide' AND validate = '1'");
			$row = $DB->fetch_row($query);
			$downloads = $row['downloads'];
			++$downloads;
		...
		";
		$navbar = "<a href=\"index.php?ind=downloads\">{$mklib->lang['dw_ptitle2']}</a>-><a href=\"#\">{$mklib->lang['dw_searchf']}</a>";
		$stat = $this->retrieve_stat();
		$toolbar = "";
		$utonline = $mklib_board->get_active_users("downloads");
		$output  = $this->tpl->downloads_show($navbar, $maintit, $content, $submit, $stat, $toolbar, $show_pages, $utonline);
	...
	function result_search() {
		...
		$navbar = "<a href=\"index.php?ind=downloads\">{$mklib->lang['dw_ptitle2']}</a>-><a href=\"#\">{$mklib->lang['dw_searchresult']}</a>";
