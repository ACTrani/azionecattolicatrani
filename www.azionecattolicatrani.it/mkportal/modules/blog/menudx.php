...
$contentdx = "
	...
	//Last Comments
	$contentdx = "";
	$query = mysql_query("select id, id_blog, id_post, autore, commento from mkp_blog_commenti ORDER BY 'id' DESC LIMIT 5");
	
	if (mysql_num_rows($query)) {
       		 while (list ($id, $idb, $idp, $autore, $post) = mysql_fetch_row ($query)) {
			  $post = str_replace ("<br />", " ", $post);
			  $post = $this->decode_bb($post);
			  $post = strip_tags ($post);
			  $post = stripslashes($post);
