...
class mk_blog {
	...
	<tr>
	  <td align=\"center\"><br /><br />
	    <div align=\"center\"><a href=\"http://www.mkportal.it\" target=\"_blank\">MKPBlog</a> &copy;2004-2006 <a href=\"http://www.mkportal.it\" target=\"_blank\">mkportal.it</a></div>
	  </td>
	</tr>	 
	";
	  ...
	  <td>
		...
		if ( $DB->get_num_rows() ) {
			while ( $r = $DB->fetch_row() ) {
				...
				$output .= "
					<tr>
	  				<td  width=\"100%\" align=\"center\" class=\"tdblock\" valign=\"middle\"><a href=\"javascript:add_linkim('$image')\"><img src=\"$image\" width='$thumb_mes[0]' height='$thumb_mes[1]' border=\"0\" valign=\"left\" alt=\"$image\" /></a></td>
					</tr>
				";				
			}
		}
	...
	function show_comments () {
		...
		$query = mysql_query("select id, autore, home, commento, ipaddress, data from mkp_blog_commenti where id_post = '$idm' ORDER BY id DESC");
        while (list ($idcom, $autore, $home, $commento, $ipaddress, $data) = mysql_fetch_row ($query)) {
            $ipcomm = "";
		...
		if ($commento) {
			if ($row['mailcomm'] == 1) {
				...
				$mklib_board->simple_mail($mklib->lang['b_mailcomms'], $mailtext, $id_blog);	
       			...
       			mysql_query("insert into mkp_blog_commenti(id_blog, id_post, autore, home, commento, ipaddress, data) values ('$id_blog', '$idm', '$autore', '$home', '$commento', '$ipaddress', '$data')");
	...
	function edit_blog () {
		...
		$query = mysql_query("select titolo, descrizione, eta, segno, citta, libri, film, canzoni, amo, odio, citazione, maxmess, anon_comm, mailcomm from mkp_blog where id = '$idu'");
		list ($titolo, $descrizione, $eta, $segno, $citta, $libri, $film, $canzoni, $amo, $odio, $citazione, $maxmess, $anon_comm, $mailcomm) = mysql_fetch_row ($query);
	...
	function edit_save () {
		...
		mysql_query("update mkp_blog set titolo = '$titolo', descrizione = '$descrizione', eta = '$eta', segno = '$segno', citta = '$citta', libri = '$libri', film = '$film', canzoni = '$canzoni', amo = '$amo', odio = '$odio', citazione = '$citazione', maxmess = '$maxmess', anon_comm = '$anon_comm', mailcomm = '$mailcomm'  where id = '$idb'");
	...
	if ($singlepost) {
		$query1 = mysql_query("select id, post, data, ncom from mkp_blog_post where id = '$singlepost' AND id_blog = '$idu'");
	} else  {
		$query1 = mysql_query("select id, post, data, ncom from mkp_blog_post where id_blog = '$idu' ORDER BY id DESC");
	}
        list ($username, $titolo, $descrizione, $template, $template2, $eta, $segno, $citta, $libri2, $film2, $canzoni2, $link2, $amo2, $odio2, $umore, $citazione, $click, $bloglink2, $maxmess) = mysql_fetch_row($query);
...
//-- magic words end
	...
	function p_gal () {
		...
		$full = "";
		//check if has a blog.
		$query = $DB->query("select id, validate from mkp_blog where id = '$idu'");
...
$content.= "<td><table border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"0\"><tr>";
		...
		<tr>
		  ...
		  <td> 
			...
			<tr>
			  <td colspan=\"2\"><input type=\"submit\" value=\"{$mklib->lang['b_addfile']}\" class=\"bgselect\" /></td>
			</tr>
		      </table>
		      </form>
		      </td>
		...
		$mklib->printpage_blog("1", "1", "{$mklib->lang['b_pgaltit']}", $output);
	}
	function upload_imm () {
	...
	function update_last() {
		...
		$DB->query("UPDATE mkp_stat SET valore ='$row[post]' where chiave = 'blog_post'");
		$query = $DB->query( "select titolo from mkp_blog where id = '$idblog'");
		$row = $DB->fetch_row($query);
		$DB->query("UPDATE mkp_stat SET valore ='$row[titolo]' where chiave = 'blog_titolo'");
