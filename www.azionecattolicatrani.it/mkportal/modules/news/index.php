...
*/
    ...
    function news_show_category() {
		...
		$query = $DB->query("SELECT id FROM mkp_news WHERE idcategoria = '$idc' AND validate = '1'");
		$count = mysql_num_rows ($query);
 ...
 function news_show_single() {
	...
	$query = $DB->query( "SELECT id, idcategoria, idautore, titolo, autore, testo, data FROM mkp_news where id = '$ide' AND validate = '1'");
	...
	if(!$row) {
		...
		$query2 = $DB->query("SELECT titolo FROM mkp_news_sections where id = '$idcat'");
		$row2 = $DB->fetch_row($query2);
		$titolocategoria = stripslashes($row2['titolo']);
		$content .= "
		<script type=\"text/javascript\">
			function makesure2() {
			if (confirm('{$mklib->lang[ne_delneconfirm]}')) {
	...
	<tr>
	  <td><br />
	    <table cellspacing=\"0\" cellpadding=\"0\" width=\"98%\" align=\"center\" border=\"0\">
	      <tr>
		<td width=\"25\"><img src=\"$mklib->images/locbar.gif\" alt=\"\" /></td>
		<td width=\"60%\"><a href=\"index.php?ind=news\">{$mklib->lang['ne_mkpnews']}</a>-><a href=\"index.php?ind=news&amp;op=news_show_category&amp;idc=$idcat\">{$titolocategoria}</a>-><a href=\"#\">$titolo</a></td>
		<td class=\"modulelite\" align=\"right\">&nbsp;&nbsp;
	";
	if($mkportals->member['g_access_cp'] || $mklib->member['g_send_news']) {
		$output .= "<a href=\"index.php?ind=news&amp;op=submit_news\">[ {$mklib->lang['ne_insertn']} ]</a>&nbsp;&nbsp;";
