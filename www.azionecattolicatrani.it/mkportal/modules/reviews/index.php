...
class mk_review {
	...
	$per_page = $mklib->config['rev_sec_page'];
	...
	function entry_view() {
		...
		$query = $DB->query( "SELECT id_cat, title, description, field1, field2, field3, field4, field5, field6, field7, image, review, author, idauth, click, rate, trate FROM mkp_reviews where id = '$iden' AND validate = '1'");
	...
	function edit_file() {
		...
		$query = $DB->query( "SELECT id_cat, title, description, field1, field2, field3, field4, field5, field6, field7, review, author, idauth FROM mkp_reviews where id = '$iden'");
		$row = $DB->fetch_row($query);
		if(!$row) {
            		$message = "{$mklib->lang['error_404']}";
            		$mklib->error_page($message);
  ...
  function update_file() {
	...
	function del_file() {
		...
		$iden= $mkportals->input['iden'];
		$query = $DB->query( "SELECT image, author, idauth FROM mkp_reviews WHERE id = $iden");
		$row = $DB->fetch_row($query);
		$file = $row['image'];
		$autore = $row['author'];
		$idauth = $row['idauth'];
  ...
  function submit_comment() {
	...
	function submit_rate() {
		...
		$t_ev1 = $row['id_cat'];
		$t_ev2 = $this->retrieve_event($t_ev1);
		$navbar = "<a href=\"index.php?ind=reviews\">{$mklib->lang['re_ptitle2']}</a>-><a href=\"index.php?ind=reviews&amp;op=section_view&amp;idev=$t_ev1\">$t_ev2</a>-><a href=\"index.php?ind=reviews&amp;op=entry_view&amp;iden=$t_id\">$t_t</a>-><a href=\"#\">{$mklib->lang['re_vote']}</a>";
	   $content .= "
		<tr>
		  <td class=\"modulecell\">
	...
	function result_search() {
		...
		$content = $this->tpl->row_main_entries();
		$query = $DB->query( "SELECT id, title, description, click, date, trate FROM mkp_reviews where $campo LIKE '%$testo%' AND validate = '1'");
		while( $row = $DB->fetch_row($query) ) {
			$iden = $row['id'];
			$name = $row['title'];
			$description = $row['description'];
		...
		function retrieve_stat() {
			...
			$query = $DB->query( "SELECT id, title FROM mkp_reviews WHERE validate = '1' ORDER BY `click` DESC LIMIT 1");
			$row = mysql_fetch_array($query);
			$id = $row['id'];
			$name = $row['title'];
			$visitato = "<a href=\"index.php?ind=reviews&amp;op=entry_view&amp;iden=$id\">$name</a>";
			$query = $DB->query( "SELECT id, title FROM mkp_reviews WHERE validate = '1' ORDER BY `trate` DESC LIMIT 1");
			$row = mysql_fetch_array($query);
			$id = $row['id'];
			$name = $row['title'];
