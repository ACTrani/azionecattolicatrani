	...
	function popup_pm($m1, $m2, $m3, $m4)
 	{
		...
		$DB->query("UPDATE " . TABLE_PREFIX . "users SET  unreadpms='0' WHERE uid={$mkportals->member['id']}");
...
//blocks functions
	...
	function forum_link($link)
	{
		...
		switch($link) {
    		...
    		break;
			case 'logout':
    			$out = "{$mkportals->forum_url}/member.php?action=logout&amp;uid=".$idu;
    		break;
			case 'postlink':
	...
	function get_forumnav()
 	{
		...
		$out = "
				<tr><td width=\"100%\" class=\"tdblock\"><img src=\"$mklib->images/atb_npost.gif\" align=\"left\" alt=\"\" />&nbsp;<a class=\"uno\" href=\"$mkportals->forum_url/search.php?action=getnew\">{$mklib->lang['m_newpost']}</a></td></tr>
				...
				<tr><td width=\"100%\" class=\"tdblock\"><img src=\"$mklib->images/atb_help.gif\" align=\"left\" alt=\"\" />&nbsp;<a class=\"uno\" href=\"$mkportals->forum_url/misc.php?action=help\">{$mklib->lang['m_help']}</a></td></tr>
     	 	";
			return $out;
	...
	function get_board_news()
 	{
		...
		WHERE p.fid IN (".implode(',', $forum_active ).") AND p.visible=1
		...
		$out .= "
					...
					<br /><div align=\"right\" style='font-style: italic; font-weight: normal;'><a href=\"$mkportals->forum_url/showthread.php?tid=$tid&amp;pid=$pid#pid$pid\">$numreplies</a>&nbsp;</div>
					</td>
				      </tr>
				      <tr>
					<td colspan=\"2\"><br />
