...
function mkportal_board_out($output) {
	...
	if($FORUM_VIEW == 1 && $mybb->input['action'] != "buddypopup" && !$mybb->input['popup'] && !$NOOUT) {
		...
		$mkportals->member['id'] = $mybb->user['uid'];
		...
		$mkportals->member['mgroup'] = $mybb->user['usergroup'];
		...
		$output = str_replace ("<div id=\"container\">", "<div style=\"padding: 5px; text-align: left;\">", $output);
