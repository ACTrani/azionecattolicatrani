...
//common functions
	...
	function store_location($loc) {
        	...
        	$DB->query("UPDATE " . TABLE_PREFIX . "session SET location ='$location'  where idhash = '$idu'");
	}
	...
	function get_forumnav()
 	{
		...
		$out = "
				<tr><td width=\"100%\" class=\"tdblock\"><img src=\"$mklib->images/atb_npost.gif\" align=\"left\" alt=\"\" />&nbsp;<a class=\"uno\" href=\"$mkportals->forum_url/search.php?do=getnew\">{$mklib->lang['m_newpost']}</a></td></tr>
				<tr><td width=\"100%\" class=\"tdblock\"><img src=\"$mklib->images/atb_members.gif\" align=\"left\" alt=\"\" />&nbsp;<a class=\"uno\" href=\"$mkportals->forum_url/memberlist.php\">{$mklib->lang['m_users']}</a></td></tr>
				<tr><td width=\"100%\" class=\"tdblock\"><img src=\"$mklib->images/atb_calendario.gif\" align=\"left\" alt=\"\" />&nbsp;<a class=\"uno\" href=\"$mkportals->forum_url/calendar.php\">{$mklib->lang['m_calendar']}</a></td></tr>
				<tr><td width=\"100%\" class=\"tdblock\"><img src=\"$mklib->images/atb_help.gif\" align=\"left\" alt=\"\" />&nbsp;<a class=\"uno\" href=\"$mkportals->forum_url/faq.php\">{$mklib->lang['m_help']}</a></td></tr>
     	 	";
			return $out;
...
");
	...
	function get_board_news()
 	{
		global $DB, $mklib, $mkportals, $db_prefix, $user_info, $modSettings, $vbulletin;
		...
		$out .= "
					...
					<br /><div align=\"right\" style='font-style: italic; font-weight: normal;'><a href=\"$mkportals->forum_url/showthread.php?t=$tid\">$numreplies</a>&nbsp;</div>
					</td>
				      </tr>
				      <tr>
					<td colspan=\"2\"><br />
	...
	function langselect() {
		global $DB, $mklib, $mkportals, $bbuserinfo;
		...
		$DB->query("SELECT languageid, title from " . TABLE_PREFIX . "language WHERE userselect='1'");
		while ( $r = $DB->fetch_row() ) {
			$selected = "";
			$name = $r['title']; 
			...
			$content .= "\n<option value=\"$mklib->siteurl/index.php?langid={$r['languageid']}\" $selected >$name</option>";		
	...
	if (!$mkportals->member['id']) {
        ...
        $DB->query("SELECT languageid from " . TABLE_PREFIX . "language WHERE languageid = '$langid'");
        if ($DB->fetch_row()){
            $DB->query("UPDATE " . TABLE_PREFIX . "user SET languageid ='$langid'  where  userid = '$idu'");
            $DB->close_db();
