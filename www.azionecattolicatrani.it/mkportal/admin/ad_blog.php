...
class mk_ad_blog {
	...
	global $mkportals, $mklib, $Skin, $DB;
		...
		//Refresh Static Blog Redirect Pages
        	...
        	$checksave = "{$mklib->lang['ad_refblog']}<br /><br />";
           	}
	...
	<tr>
	  ...
	  <script type=\"text/javascript\">
	    ...
	    <form action=\"admin.php?ind=ad_blog&amp;op=refresh_static\" name=\"refresh_static\" method=\"post\">
	    <table width=\"100%\" border=\"0\">  
	      <tr>
		<td>{$mklib->lang['ad_blogrefresh']}</td>
	      </tr>
	      <tr>
		<td><br /><input type=\"submit\" name=\"regenerate\" value=\"{$mklib->lang['ad_refsubmit']}\" class=\"bgselect\"  onclick=\"return makesurerefresh()\" /></td>
	      </tr>
	    </table>
	    </form>
...
//Refresh Static Blog Redirect Pages
    function refresh_static() {
		...
		while( $row = $DB->fetch_row($query) ) {
         ...
         Header("Location: admin.php?ind=ad_blog&mode=refresh_static");
        exit;
        }
//end Refresh Static
