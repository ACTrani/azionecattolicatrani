...
function parse_bbc($message, $smileys = true, $cache_id = '')
{
	...
	// - If disableautoembedvideo is set (eg for signatures)
	// - If there are no links
	// - If we've already exceeded our limits
	if($smileys !== "print" && empty($context['disableautoembedvideo']) && stripos($message, '<a href="') !== false && (!isset($autoembedmax) || $autoembedmax != 0))
	{
		global $sourcedir;
		@require_once($sourcedir.'/Subs-AutoEmbedVideoClips.php');
		if(function_exists('AutoEmbedVideoClips'))
			$message = AutoEmbedVideoClips($message);
	}
	elseif(stripos($message, '[noembed]') !== false)
	{
		// Even if we aren't calling the function, we may still need to parse out [noembed] bbcode
		global $sourcedir;
		@require_once($sourcedir.'/Subs-AutoEmbedVideoClips.php');
		if(function_exists('StripAndFixNoEmbed'))
